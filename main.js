...
const scale = 3;
const camera = new THREE.OrthographicCamera(-scale, scale, scale * aspectr, -scale * aspectr, 0.1, 100);
...
function insertBox(u, v) {
    let l = Math.random();
    let cube = new THREE.Mesh(new THREE.BoxGeometry(boxSet.size, boxSet.size, boxSet.size), new THREE.MeshPhongMaterial({ color: new THREE.Color(l, l, l) }));
    let pos = boxPos(u, v);
...
/* Inverse Kinematics */
class IK {
    constructor(color, joints, lengths) {
        ...
        this.draw_data = new THREE.Line(new THREE.BufferGeometry().setFromPoints(joints), new THREE.LineBasicMaterial({ color: color }));
...
function testIK() {
    let joints_count = 10;
    let arm_length = 2;
    let joints = Array(joints_count)
        .fill(new THREE.Vector3())
        .map((j, i) => new THREE.Vector3(0, i * arm_length / joints_count, 0));
    let lengths = Array(joints_count - 1).fill(arm_length / joints_count);
    console.log(joints);
    console.log(lengths);
    ik = new IK(new THREE.Color(0, 0, 0), joints, lengths);
...
class Robot {
    constructor() {
        ...
        this.ik = new IK(new THREE.Color(0, 0, 0), joints, lengths);
    ...
    pickNewTarget() {
        ...
        this.insertTarget = boxPos(this.uTarget, this.vTarget);
        this.t = 0;
    }
    update() {
        this.t += 0.03;
        if (this.t >= 1.0) {
            insertBox(this.uTarget, this.vTarget);
            this.pickNewTarget();
        }
